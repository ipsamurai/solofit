...
export function calculateReadiness(presageData, userData) {
	if (!presageData) {
		return {
			...
			details: "No biometric data available",
		};
	}

	const { heart_rate, breathing_rate, stress_level, source } = presageData;
	...
	if (
		userData?.healthIssues?.length > 0 &&
		!userData.healthIssues.includes("None")
	) {
		score -= 5;
	}
	...
	if (score >= 75) {
		...
		recommendation = "rest";
		details =
			"Your body needs recovery. Focus on stretching or take the day off.";
	}
