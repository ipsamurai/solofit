...
function analyzePose() {
  ...
  const poses = [
    { posture: 'good', angle: 172, feedback: 'Great form! Keep your back straight.' },
    { posture: 'fair', angle: 155, feedback: 'Slight forward lean detected. Engage your core more.' },
    { posture: 'poor', angle: 130, feedback: 'Significant slouching. Risk of lower back strain.' },
  ];
...
export default function PostureScreen() {
  ...
  const showFatigueWarning = result?.posture === 'poor' && readiness?.readiness_score < 45;

  return (
    <SafeAreaView style={styles.safe}>
      <View style={styles.container}>
        <Text style={styles.title}>Posture Check</Text>
        <Text style={styles.subtitle}>Camera-based form analysis using pose detection</Text>
        ...
        <View style={styles.cameraArea}>
          <View style={styles.cameraPlaceholder}>
            ...
            <Text style={styles.cameraText}>
              {analyzing ? 'Analyzing posture...' : 'Position yourself in frame'}
            </Text>
          </View>

          {/* Pose Overlay Visualization */}
          {result && (
            <View style={[styles.postureBadge, { backgroundColor: getPostureColor(result.posture) + '20' }]}>
              <Text style={[styles.postureText, { color: getPostureColor(result.posture) }]}>
        ...
        {result && (
          <Card style={styles.resultCard}>
            <View style={styles.resultHeader}>
              <Ionicons
                name={result.posture === 'good' ? 'checkmark-circle' : result.posture === 'fair' ? 'alert-circle' : 'close-circle'}
                size={24}
            ...
            <View style={styles.angleRow}>
              ...
              <Text style={[styles.angleValue, { color: getPostureColor(result.posture) }]}>
                {result.angle}°
              </Text>
            </View>
          </Card>
            ...
            <View style={styles.warningRow}>
              ...
              <View style={styles.warningContent}>
                ...
                <Text style={styles.warningText}>
                  Poor posture combined with high fatigue. Consider stopping the exercise to prevent injury.
                </Text>
              </View>
            </View>
          </Card>
        ...
        {result && (
          <Card style={styles.jointsCard}>
            ...
            <View style={styles.jointsGrid}>
              {['Shoulders', 'Hips', 'Knees', 'Ankles', 'Spine', 'Neck'].map((joint, i) => (
                <View key={i} style={styles.jointItem}>
                  <View style={[styles.jointDot, { backgroundColor: getPostureColor(result.posture) }]} />
...
const styles = StyleSheet.create({
  ...
  postureText: { fontSize: 12, fontWeight: '800' },
  ...
  angleRow: { flexDirection: 'row', justifyContent: 'space-between', marginTop: SPACING.md },
  angleLabel: { ...FONTS.bodySmall },
  angleValue: { fontSize: 20, fontWeight: '700' },
  warningCard: { marginTop: SPACING.sm, borderLeftWidth: 3, borderLeftColor: COLORS.danger },
  warningRow: { flexDirection: 'row', gap: SPACING.sm },
  warningContent: { flex: 1 },
  warningTitle: { ...FONTS.body, fontWeight: '700', color: COLORS.danger },
  warningText: { ...FONTS.bodySmall, marginTop: 2 },
  jointsCard: { marginTop: SPACING.sm },
  jointsTitle: { ...FONTS.body, fontWeight: '600', marginBottom: SPACING.sm },
  jointsGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: SPACING.md },
  jointItem: { flexDirection: 'row', alignItems: 'center', gap: SPACING.xs, width: '45%' },
