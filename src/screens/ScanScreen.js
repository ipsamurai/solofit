...
export default function ScanScreen() {
  ...
  const handleStartScan = useCallback(async () => {
    ...
    if (!micPermission?.granted) {
      ...
      if (!result.granted) {
        setErrorMsg('Microphone permission is required to record video for vitals analysis.');
        setPhase(PHASE.ERROR);
        return;
      }
    }
    ...
    try {
      ...
      clearInterval(recordTimer.current);
      console.warn('[ScanScreen] Recording warning (will use fallback):', err.message);
    }
    ...
    try {
      const vitalsData = await analyzeVideoForVitals(videoUri, {
        onProgress: (msg) => {
          setProgressMsg(msg);
          if (msg.toLowerCase().includes('analyz') || msg.toLowerCase().includes('estimat')) {
  ...
  const showCamera = permission?.granted && micPermission?.granted && !isDone && !isError;
      ...
      >
            ...
            >
              ...
              {isProcessing && (
                <View style={styles.processingOverlay}>
                  <Ionicons name="pulse-outline" size={44} color={COLORS.primary} />
                  <Text style={styles.processingText}>{progressMsg || 'Analyzing...'}</Text>
                </View>
              )}
            </CameraView>
        ...
        {(!permission?.granted || !micPermission?.granted) && !isDone && (
          <View style={styles.permBox}>
            ...
            <Text style={styles.permDesc}>
              Solofit needs access to your camera and microphone to record a short face scan and measure your heart rate and breathing rate.
            </Text>
            <TouchableOpacity
            ...
            >
              ...
              <Text style={styles.primaryBtnText}>Allow Camera & Microphone</Text>
            </TouchableOpacity>
          </View>
        )}
        ...
        {isDone && vitals && (
          <View style={styles.results}>
            ...
            <View style={styles.sourceBadge}>
              <Ionicons
                name={vitals.source === 'presage_api' ? 'checkmark-circle' : 'information-circle'}
                size={14}
                color={vitals.source === 'presage_api' ? COLORS.success : COLORS.warning}
              />
              <Text
                style={[
                  styles.sourceText,
              ...
              >
                {vitals.source === 'presage_api' ? 'Presage API — Live Data' : 'Estimated Vitals'}
              </Text>
            </View>
            ...
            <View style={[styles.vitalsRow, { marginTop: SPACING.sm }]}>
              <VitalCard
                ...
                color={
                  vitals.stress_level === 'high' ? COLORS.danger
                    : vitals.stress_level === 'medium' ? COLORS.warning
                    : COLORS.success
                }
                style={{ flex: 1 }}
              />
        ...
        <View style={styles.buttonArea}>
          {isBusy ? (
            ...
            <TouchableOpacity
              style={[styles.primaryBtn, (isError || isDone) && styles.retryBtn]}
          ...
          {isProcessing && (
            <View style={styles.processingIndicator}>
              ...
              <Text style={styles.processingIndicatorText}>{progressMsg || 'Processing...'}</Text>
            </View>
          )}
        </View>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  idleHintText: { color: 'rgba(255,255,255,0.8)', fontSize: 13, fontWeight: '500' },
  ...
  countdownNum: { fontSize: 100, fontWeight: '900', color: '#fff', lineHeight: 110 },
  countdownSub: { fontSize: 16, color: 'rgba(255,255,255,0.7)', fontWeight: '500' },
  ...
  processingText: { fontSize: 15, color: COLORS.textSecondary, textAlign: 'center', paddingHorizontal: 24 },
  ...
  sourceBadge: { flexDirection: 'row', alignItems: 'center', gap: 5, marginBottom: SPACING.md },
  ...
  cancelText: { ...FONTS.bodySmall, color: COLORS.textSecondary, fontWeight: '600' },
  ...
  processingIndicatorText: { ...FONTS.bodySmall, color: COLORS.primary, fontWeight: '600' },
});
