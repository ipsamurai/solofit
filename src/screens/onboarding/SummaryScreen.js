    ...
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content}>
        ...
        <Card style={styles.card}>
          ...
          <SummaryRow icon="scale" label="Weight" value={d.weight ? `${d.weight} kg` : null} />
          <SummaryRow icon="resize" label="Height" value={d.height ? `${d.height} cm` : null} />
        </Card>

        <Card style={styles.card}>
          <Text style={styles.sectionTitle}>Goals & Lifestyle</Text>
          <SummaryRow icon="trophy" label="Goals" value={d.goals?.join(', ')} />
          <SummaryRow icon="speedometer" label="Activity" value={d.activityLevel} />
        </Card>

        <Card style={styles.card}>
          <Text style={styles.sectionTitle}>Diet</Text>
          <SummaryRow icon="restaurant" label="Type" value={d.dietType} />
          <SummaryRow icon="warning" label="Allergies" value={d.allergies?.join(', ') || 'None'} />
        </Card>

        <Card style={styles.card}>
          <Text style={styles.sectionTitle}>Health & Equipment</Text>
          <SummaryRow icon="medkit" label="Issues" value={d.healthIssues?.join(', ') || 'None'} />
          <SummaryRow icon="barbell" label="Equipment" value={d.equipment?.join(', ') || 'None'} />
        </Card>
...
const styles = StyleSheet.create({
  ...
  rowValue: { ...FONTS.body, fontWeight: '500', flex: 1, textAlign: 'right' },
