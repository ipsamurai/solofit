...
async function uploadChunks(fileBuffer, urls) {
  ...
  for (let i = 0; i < urls.length; i++) {
    ...
    if (!res.ok) {
      throw new Error(`S3 chunk upload ${i + 1}/${urls.length} failed (${res.status})`);
    }

    // S3 returns ETag in response headers
    const etag = res.headers.get('ETag') || res.headers.get('etag') || `"part${i + 1}"`;
...
async function pollForResults(id, apiKey, onProgress) {
  for (let attempt = 0; attempt < MAX_POLL_ATTEMPTS; attempt++) {
    ...
    if (res.status === 201) {
      ...
      console.log(`[Presage] Still processing (attempt ${attempt + 1}/${MAX_POLL_ATTEMPTS})`);
      continue;
    }

    if (res.status === 401) throw new Error('Invalid Presage API key (401 on poll)');
...
function parseResults(data) {
  ...
  if (data.rr && typeof data.rr === 'object') {
    const rrVals = Object.values(data.rr).filter((v) => typeof v === 'number' && v > 0);
    if (rrVals.length) {
...
export async function analyzeVideoForVitals(videoUri, { onProgress } = {}) {
  ...
  try {
    ...
    onProgress?.(`Uploading scan (${urls.length} part${urls.length > 1 ? 's' : ''})...`);
    ...
    console.error('[Presage] Error, falling back to mock:', err.message);
    onProgress?.('Using estimated vitals (API unavailable)');
